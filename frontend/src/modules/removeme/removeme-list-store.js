...
import removemeListExporterFields from '@/modules/removeme/removeme-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!removemeListExporterFields || !removemeListExporterFields.length) {
          throw new Error('removemeListExporterFields is required');
        }
        ...
        const response = await RemovemeService.list(
          filter,
          getters.orderBy,
          null,
          null,
