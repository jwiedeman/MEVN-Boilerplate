  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await RemovemeService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.removeme.create.success'));
        routerAsync().push('/removeme');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await RemovemeService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.removeme.update.success'));
        routerAsync().push('/removeme');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
